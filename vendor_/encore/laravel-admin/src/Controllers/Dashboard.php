...
class Dashboard
{
    ...
    public static function latestContent()
    {
       // $boardContentModel = BlotBoardContent::class;        
        $threeDaysAgo = now()->subDays(30)->format('Y-m-d H:i:s');
        $results = BlotBoardContent::where('created_at', '>=', $threeDaysAgo)->get(); 
        return view('admin::dashboard.latestContent', ['results'=> $results ] );
    }
    public static function latestRequest()
    {
       // $boardContentModel = BlotBoardContent::class;        
        $threeDaysAgo = now()->subDays(30)->format('Y-m-d H:i:s');
        $results = BlotContact::where('created_at', '>=', $threeDaysAgo)->get(); 
        return view('admin::dashboard.latestRequest', ['results'=> $results ] );
    } 
    /**
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
